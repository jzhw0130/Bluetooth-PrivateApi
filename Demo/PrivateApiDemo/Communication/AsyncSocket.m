...
- (void)runLoopAddTimer:(NSTimer *)timer
{
	...
	{
		CFStringRef runLoopMode = (__bridge CFStringRef)[theRunLoopModes objectAtIndex:i];
		CFRunLoopAddTimer(theRunLoop, (CFRunLoopTimerRef)CFBridgingRetain(timer), runLoopMode);
	}
}
...
- (void)runLoopRemoveTimer:(NSTimer *)timer
{
	...
	{
		CFStringRef runLoopMode = (__bridge CFStringRef)[theRunLoopModes objectAtIndex:i];
		CFRunLoopRemoveTimer(theRunLoop, (CFRunLoopTimerRef)CFBridgingRetain(timer), runLoopMode);
	}
}
...
- (BOOL)setRunLoopModes:(NSArray *)runLoopModes
{
	...
	if(theReadStream && theWriteStream)
	{
		if(![self attachStreamsToRunLoop:(NSRunLoop *)CFBridgingRelease(theRunLoop) error:nil])
		{
			return NO;
		}
	}
...
- (BOOL)acceptOnAddress:(NSString *)hostaddr port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket)
	{
		err = CFSocketSetAddress (theSocket, (CFDataRef)CFBridgingRetain(address));
	...
	if (theSocket6)
	{
		err = CFSocketSetAddress (theSocket6, (CFDataRef)CFBridgingRetain(address6));
		if (err != kCFSocketSuccess) goto Failed;
...
- (BOOL)connectSocketToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if(theSocket)
	{
		CFSocketError err = CFSocketConnectToAddress(theSocket, (CFDataRef)CFBridgingRetain(remoteAddr), -1);
	...
	{
		CFSocketError err = CFSocketConnectToAddress(theSocket6, (CFDataRef)CFBridgingRetain(remoteAddr), -1);
		if(err != kCFSocketSuccess)
		{
			if (errPtr) *errPtr = [self getSocketError];
			return NO;
...
{
	// Create the socket & streams.
	CFStreamCreatePairWithSocketToHost(kCFAllocatorDefault, (CFStringRef)CFBridgingRetain(hostname), port, &theReadStream, &theWriteStream);
	if (theReadStream == NULL || theWriteStream == NULL)
	{
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		AsyncSocket *socket = (AsyncSocket *)CFBridgingRelease(pInfo);
		[socket doCFSocketCallback:type forSocket:sref withAddress:(NSData *)CFBridgingRelease(address) withData:pData];
